...
@push('myscript')
    <script>
        ...
        function successCallback(position) {
            ...
            // Cek apakah $lok_absen ada dan valid sebelum digunakan
            @if ($lok_absen)
                var lokasi_absen = "{{ $lok_absen->lokasi_absen }}";
                var lok = lokasi_absen.split(",");
                var lat_absen = lok[0];
                var long_absen = lok[1];
                var radius_absen = "{{ $lok_absen->radius }}";
            @else
                // Set nilai default jika $lok_absen tidak tersedia
                var lat_absen = 0;
                var long_absen = 0;
                var radius_absen = 100; // Radius default jika tidak ada radius yang ditetapkan
            @endif
        ...
        function errorCallback() {
            console.error("Lokasi tidak tersedia atau akses ditolak.");
        }
